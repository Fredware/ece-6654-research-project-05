...
tc_app.CollectingBaselineLamp.Color = 'red';
baseline_period = 3; %seconds
fs = 1000; % Hz

% Retrieve the latest data from buffer
emg = uno.getRecentEMG; % value range: [-2.5:2.5]; length range: [1:buffer length]

% Compute detection threshold using latest samples of retrieved data
baseline_data = emg(end-fs*baseline_period:end);
...
% while( ishandle(fig))
while( ishandle(tc_app.UIFigure))
    % SAMPLE ARDUINO
    pause(0.0111111)
    try
        emg = uno.getRecentEMG; % value range: [-2.5:2.5]; length range: [1:buffer length]
...
%         if mav_feat > mav_thresh && features_idx > 10
%             plot(tc_app.UIAxes_mav_event, t_features(features_idx-10:features_idx), features(features_idx-10:features_idx));
%         end
